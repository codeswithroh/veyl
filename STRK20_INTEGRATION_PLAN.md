...
**What's verified (updated 2026-08-24):** build/typecheck clean, live-rendered against the real deployed Sepolia round — the Launch tab correctly reads `get_round` on-chain and shows live phase/ticket/supply data. Beyond that, a full `create_round → commit → reveal → finalize → claim` round has now been run for real against the live Sepolia STRK20 pool end to end, **claim included** (`is_claimed == true` on-chain) — see `cairo/README.md` "Live pool-mediated round — full verification" for every tx hash. That live run caught and fixed three real bugs beyond what unit tests had covered: `_claim` reverting on a full-fill round's exactly-zero refund (`ZERO_AMOUNT`), `_claim` under-approving the pool's own per-open-note fee (`Insufficient ERC20 allowance`), and `_commit`'s exact-match escrow-delta assert breaking under a pre-funded fee buffer (`AMOUNT_MISMATCH`).

**What isn't yet verified:** the round above was driven by scripts holding the account key directly (`server/scripts/fair-launch-*.ts`), not through an actual connected wallet extension (Ready) via `WalletAccountV6Tag.tsx`'s wallet-connect flow — this dev environment still has no wallet extension to test against. The action shapes match (withdraw+invoke for commit; transfer+invoke for claim), and the frontend's claim handler was updated to match the fixed contract, but a manual pass with a real Ready wallet on Sepolia is still worth doing before trusting it in front of real users. Also unverified live: an oversubscribed (pro-rata, nonzero-refund) round — only full-fill has been run against the real pool so far.
